#ifndef pta_CorPTAProcessor_h
#define pta_CorPTAProcessor_h
#include <Rcpp.h>
#include <vector>
#include <cassert>
...
class CorPTAProcessor {
    private:
        ...
        struct NodeGreater {
            const CorPTAProcessor* processor;
            int heap;
            NodeGreater() : processor(NULL), heap(-1) {}
            void set(const CorPTAProcessor* p, int h) {
                processor = p;
                heap = h;
            }
    ...
    public:
        CorPTAProcessor(SEXP start_, SEXP end_, SEXP scores_, SEXP count_, SEXP error_, SEXP adjacency_treshold_, SEXP skip_, SEXP mode_);
        void run();
